        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    bytes = mmInStream.read(buffer, bytes, 1);
                    bytes += mmInStream.read(buffer, bytes, 1);
                    bytes += mmInStream.read(buffer, bytes, 1);
                    Log.d(TAG,"" + buffer[0]);
                    if(buffer[0] == -39 && buffer [1] == -39 && buffer[2] == -39)
                    {
                        bytes += mmInStream.read(buffer, bytes, 1);
                        bytes += mmInStream.read(buffer, bytes, 1);
                        bytes += mmInStream.read(buffer, bytes, 1);
                        Log.d(TAG, "buffer:" + buffer[2] +"  " + buffer[5]);
                    }

                    if (bytes == 6 && buffer[2] == -39 && buffer[5] == -39) {

                        for (int i = 3; i < bytes-1; i++) {
                            sb.append(String.format("%02X", buffer[i]));
                        }
                        Log.d(TAG, "Image Size: " + sb);

                        byte1 = buffer[3] & 0xFF;
                        byte2 = buffer[4] & 0xFF;
                        Log.d(TAG, "Byte 1:" + byte1);
                        Log.d(TAG, "Byte 2:" + byte2);
                        length = byte1 << 8;
                        length += byte2 & 0xFF;
                        ...
                        Log.d(TAG, "buffer byte length:" + bytes);
